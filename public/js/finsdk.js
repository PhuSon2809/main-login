  ...
  class FinSdk {
    ...
    #initFrame({ onProgress, onFinish, onError }) {
      ...
      window.addEventListener('message', async (event) => {
        if (event.origin != this.#srcSt && event.origin != this.#srcEx && event.origin != this.#srcHandleFile) {
          return
        }
        ...
        if (
          ...
          console.log(`Exxx call ${command} -> result ${Object.keys(res).length}`)
          this.#send({
            iframeName: 'ex',
            message: {
              command,
